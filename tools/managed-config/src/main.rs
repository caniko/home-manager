...
fn main() {
    ...
    if invoked == "obs-studio-export-to-nix" {
        ...
        if matches!(args.get(1).map(String::as_str), Some("export-to-nix" | "plugin-inspect")) {
            args.insert(1, "obs".to_string());
        }
    }
...
fn validate_command(args: Validate) -> Result<()> {
    ...
    for file in &manifest.files {
        ...
        let expected_target = args.config_dir.join(&file.path);
        ...
        match sha256_file(&file.source) {
            ...
            Ok(hash) => errors.push(format!("{}: source sha256 mismatch: {hash}", file.path)),
            Err(err) => errors.push(format!("{}: cannot hash source: {err:?}", file.path)),
        }
...
fn should_omit(key: &str, value: &Value, redaction: &Redaction) -> bool {
    ...
    if !redaction.include_runtime && RUNTIME_KEYS.iter().any(|runtime| key == *runtime) {
        return true;
    }
